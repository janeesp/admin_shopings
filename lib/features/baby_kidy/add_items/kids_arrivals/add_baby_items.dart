...
class AddBabyItems extends StatefulWidget {
  const AddBabyItems({super.key});

  @override
  State<AddBabyItems> createState() => _AddBabyItemsState();
}

class _AddBabyItemsState extends State<AddBabyItems> {
              ...
              ElevatedButton(
                  onPressed: () {
                    if(Name_controller.text!=""&&rate_controller.text!=""
                      ...
                      FirebaseFirestore.instance.collection("baby&kids").doc("baby&kids").collection("kidesArrivals").add(
                          items.toMap()
                      ).then((value) {
                        value.update({
                          "id":value.id
