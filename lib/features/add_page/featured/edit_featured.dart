...
class EditFeatured extends StatefulWidget {
  DressModel dressModel;
   EditFeatured({super.key,required this.dressModel});

  @override
  State<EditFeatured> createState() => _EditFeaturedState();
}

class _EditFeaturedState extends State<EditFeatured> {
  ...
  void initState() {
    ...
    super.initState();
   Name_controller=TextEditingController(text: widget.dressModel.name);
    rate_controller=TextEditingController(text:widget.dressModel.rate.toString());
    Totel_controller=TextEditingController(text: widget.dressModel.total.toString());
            ...
            ElevatedButton(
                onPressed: () {
                  if(Name_controller.text!=""&&rate_controller.text!=""
                    ...
                    // );
                    FirebaseFirestore.instance.collection("featured").doc(widget.dressModel.id).update(
                      {
                        "name":Name_controller.text,
                        "rate":double.tryParse(rate_controller.text)   ,
                        "total":double.tryParse(Totel_controller.text)
